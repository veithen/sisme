...
public class ForwardTest {
    ...
    public void test() throws Exception {
        OSGiRuntime forwardServer = new OSGiRuntime();
        try {
            forwardServer.cmd("endpoint add remote-echo localhost 9001 direct");
            forwardServer.cmd("forward add local-echo remote-echo");
            forwardServer.cmd("port add 9000 local-echo");
